...
def send_command(command_str):
    ...
    # Removed the messagebox that was showing command sent
...
def calculate_forward_kinematics(theta1, theta2, theta3, theta4):
    ...
    # เลื่อนค่า z ลงมาให้ฐานอยู่ที่ z = 0
    zA -= OA
    zB -= OA
    zC -= OA
    zD -= OA
...
def inverse_kinematics_optimization(x_target, y_target, z_target, gamma, initial_guess):
    ...
    bounds = [
        ...
        LIMITS['theta2'],
        LIMITS['theta3'],
        LIMITS['theta4'],  # For theta4, you mentioned to allow wider range
    ]
    ...
    if result.success:
        ...
        # Display error in the error message label instead of a popup
...
def plot_robot_arm(theta1, theta2, theta3, theta4):
    ...
    ax.set_xlim([-450, 450])
    ax.set_ylim([-450, 450])
    ax.set_zlim([0, 450])  # เริ่มจาก z = 0
...
def home_position():
    ...
    # กำหนดตำแหน่ง home โดยตั้งค่ามุมทั้งหมดเป็น 0
    theta1, theta2, theta3, theta4 = 0, 0, 0, 0
    # อัปเดตช่องป้อนข้อมูลใน GUI ให้ตรงกับมุมใหม่
    theta1_entry.delete(0, tk.END)
    ...
    # อัปเดตค่า x, y, z, a โดยใช้การคำนวณ Forward Kinematics
    update_xyz_from_theta()
    # วาดกราฟของแขนหุ่นยนต์ที่ตำแหน่งใหม่
    plot_robot_arm(theta1, theta2, theta3, theta4)

# Function to send 'f 0 0 0 0' command
def move_to_zero():
    send_command("f 0 0 0 0")  # ส่งคำสั่งไปยัง Arduino
    # กำหนดมุมเป็น 0
    theta1, theta2, theta3, theta4 = 0, 0, 0, 0
    # อัปเดตช่องป้อนข้อมูลใน GUI
    theta1_entry.delete(0, tk.END)
    theta1_entry.insert(0, f"{theta1}")
    theta2_entry.delete(0, tk.END)
    theta2_entry.insert(0, f"{theta2}")
    ...
    # อัปเดตค่า x, y, z, a
    update_xyz_from_theta()
    # วาดกราฟของแขนหุ่นยนต์ที่ตำแหน่งศูนย์
    plot_robot_arm(theta1, theta2, theta3, theta4)
...
def open_camera():
    ...
    def update_frame():
        ...
        if ret:
            ...
            # Display error in the label instead of a popup
