...
# Corrected Inverse Kinematics function
def calculate_inverse_kinematics(x_target, y_target, z_target, gamma, elbow_up=True):
    try:
        # Calculate theta1
        theta1 = math.degrees(math.atan2(y_target, x_target))

        # Offsets due to initial links
        d_offset = AB * math.cos(alpha) + BC * math.cos(alpha)
        z_offset = OA + AB * math.sin(alpha) + BC * math.sin(alpha)

        # Planar distances to the wrist point
        r = math.sqrt(x_target**2 + y_target**2) - d_offset
        s = z_target - z_offset

        # Distances for Law of Cosines
        D = (r**2 + s**2 - CD**2 - DE**2) / (2 * CD * DE)
        D = max(min(D, 1), -1)  # Clamp D to [-1, 1]

        # Choose elbow-up or elbow-down configuration
        theta3_rad = math.acos(D)
        if not elbow_up:
            theta3_rad = -theta3_rad
        theta3 = math.degrees(theta3_rad)

        # Auxiliary angles
        beta = math.degrees(math.atan2(DE * math.sin(theta3_rad), CD + DE * math.cos(theta3_rad)))
        ...
        messagebox.showerror("Error", f"Invalid target position: {e}")
        return None
...
def update_theta_from_xyz():
    try:
        ...
        # Try both configurations to find the one that matches
        results_up = calculate_inverse_kinematics(x_target, y_target, z_target, gamma, elbow_up=True)
        results_down = calculate_inverse_kinematics(x_target, y_target, z_target, gamma, elbow_up=False)

        # Choose the solution that is closer to the previous joint angles
        if results_up and results_down:
            prev_theta1 = float(theta1_entry.get() or 0)
            prev_theta2 = float(theta2_entry.get() or 0)
            prev_theta3 = float(theta3_entry.get() or 0)
            prev_theta4 = float(theta4_entry.get() or 0)

            diff_up = sum(abs(a - b) for a, b in zip(results_up, [prev_theta1, prev_theta2, prev_theta3, prev_theta4]))
            diff_down = sum(abs(a - b) for a, b in zip(results_down, [prev_theta1, prev_theta2, prev_theta3, prev_theta4]))

            results = results_up if diff_up < diff_down else results_down
        elif results_up:
            results = results_up
        elif results_down:
...
def move_to_xyz():
    try:
        ...
        # Choose the solution that is within joint limits
        results = None
        if results_up and all(LIMITS[key][0] <= val <= LIMITS[key][1] for key, val in zip(LIMITS.keys(), results_up)):
            results = results_up
        elif results_down and all(LIMITS[key][0] <= val <= LIMITS[key][1] for key, val in zip(LIMITS.keys(), results_down)):
            results = results_down
        else:
            messagebox.showerror("Error", "No valid joint configuration found within limits.")
            return
