...
# ฟังก์ชันตรวจสอบมุมและแสดงข้อความ
def validate_all_thetas(theta_values):
    ...
    if invalid_thetas:
        error_message_label.config(text="Cannot input limit number in " + ", ".join(invalid_thetas))
        return False
    else:
...
def plot_robot_arm(theta1, theta2, theta3, theta4):
    ...
    ax.legend()
    canvas.draw()  # ใช้เพื่ออัปเดตกราฟ
...
# สร้าง Frame สำหรับด้านขวา (กราฟและข้อความแสดงข้อผิดพลาด)
right_frame = tk.Frame(root)
right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=1)

# สร้าง Label สำหรับแสดงข้อความข้อผิดพลาด
