...
from collections import deque  # For smoothing theta
...
class RoboticArmGUI:
    def __init__(self, root, arduino, kinematics):
        ...
        self.PIXELS_PER_CM = 37.00  # Adjusted calibration based on reference code
        ...
        # Buffer for smoothing theta values
        self.theta_buffer = deque(maxlen=5)
    ...
    def create_buttons(self, font_style):
            # Open Camera Button
            tk.Button(self.left_frame, text="Open Camera", command=self.open_camera,
                    font=font_style, height=2, width=25).pack(pady=(1, 1))

            # Start Detection Button
            self.detect_button = tk.Button(self.left_frame, text="Detect", command=self.start_detection,
                                       font=font_style, height=2, width=25, state=tk.DISABLED)
            self.detect_button.pack(pady=(1, 1))

            # Stop Detection Button
            self.stop_detect_button = tk.Button(self.left_frame, text="Stop Detect", command=self.stop_detection,
                                            font=font_style, height=2, width=25, state=tk.DISABLED)
            self.stop_detect_button.pack(pady=(1, 1))
    ...
    def open_camera(self):
        ...
        # Enable the "Close Camera" and "Detect" buttons
    ...
    def start_detection(self):
        if not self.camera_running:
            self.error_message_label.config(text="Open the camera first.")
            return

        self.stop_event.clear()  # Clear the stop flag
        self.detecting = True  # Set the detection flag
    ...
    def stop_detection(self):
        ...
        self.detecting = False  # Clear the detection flag
        self.stop_detect_button.config(state=tk.DISABLED)
        self.detect_button.config(state=tk.NORMAL)
        self.error_message_label.config(text="Detection stopped.")

    def detect_objects(self):
        self.error_message_label.config(text="Detecting objects...")
        while self.detecting and not self.stop_event.is_set():
            ...
            # Display detection results in the live feed
    ...
    def close_camera(self):
        ...
        # Disable buttons related to the camera
    ...
    def move_to_zero(self):
        self.arduino.send_command("f 90 90 -120 -60")
        # Update GUI entries
        ...
        self.plot_robot_arm(90, 90, -120, -60)
    ...
    def run_camera(self):
        ...
        CAMERA_FOV = self.CAMERA_FOV  # Using the adjusted FOV

        # Define a threshold for stopping (in degrees)
        THETA_THRESHOLD = self.THETA_THRESHOLD

        # Command rate limiting
        last_command_time = 0
        command_interval = self.command_interval  # seconds

        # Delay before starting object detection
        start_time = time.time()
        delay_duration = 5  # 5-second delay
        while time.time() - start_time < delay_duration:
            ...
            # Display the live camera feed during the delay
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            img = Image.fromarray(frame_rgb)
            img_tk = ImageTk.PhotoImage(image=img)
        ...
        while self.camera_running and not self.stop_event.is_set():
            ...
            if current_time - last_command_time > command_interval:
                ...
                if red_center:
                    ...
                    x_diff = center_x - red_center[0]  # Correct calculation
                    theta = self.compute_theta(x_diff, frame_width, fov=CAMERA_FOV)
                    logging.info(f"Red Circle - x_diff: {x_diff}, theta: {theta}")
                    self.theta_buffer.append(theta)
                    theta_avg = sum(self.theta_buffer) / len(self.theta_buffer)
                    if abs(theta_avg) > THETA_THRESHOLD:
                        self.send_command_to_robot(theta_avg)
                    else:
                ...
                if blue_object:
                    ...
                    x_diff = center_x - x_center  # Correct calculation
                    theta = self.compute_theta(x_diff, frame_width, fov=CAMERA_FOV)
                    logging.info(f"Blue Rectangle - x_diff: {x_diff}, theta: {theta}")
                    self.theta_buffer.append(theta)
                    theta_avg = sum(self.theta_buffer) / len(self.theta_buffer)
                    if abs(theta_avg) > THETA_THRESHOLD:
                        self.send_command_to_robot(theta_avg)
                ...
                # If any object is detected, reset stopped flag
                if red_center or blue_object:
                    ...
                    if not self.stopped:
                        ...
                        logging.info("No objects detected. Sent stop command.")
                        self.stopped = True
            ...
            # Calculate elapsed time and sleep accordingly to maintain frame rate
            frame_delay = 1 / 30  # ~0.033 seconds for 30 FPS
    ...
    def process_frame_queue(self):
        """Processes frames from the camera frame queue and updates the camera feed."""
        try:
            while True:
                img_tk = self.frame_queue.get_nowait()  # Retrieve the next frame from the queue
                self.camera_label.config(image=img_tk)  # Update the image in the GUI
                self.camera_label.image = img_tk       # Keep a reference to avoid garbage collection
        except queue.Empty:
            pass  # If the queue is empty, do nothing
        
        # Schedule the next frame processing if the camera is still running
        if self.camera_running:
            self.root.after(10, self.process_frame_queue)
    ...
    def send_command_to_robot(self, theta):
        ...
        """
        theta_int = int(round(theta))  # Convert to integer
        command = f"1 {theta_int}"
        try:
            # Send theta as is initially
            self.arduino.send_command(command)
            logging.info(f"Sent command to robot: {command}")
        except Exception as e:
