...
class RoboticArmGUI:
    def __init__(self, root, arduino, kinematics):
        ...
        self.stopped = True  # Flag to track if robot arm is stopped
    ...
    def create_input_fields(self, font_style):
        ...
        for i, theta in enumerate(["Theta 1", "Theta 2", "Theta 3", "Theta 4"], start=1):
            ...
            # Frame to hold entry and buttons
            ...
            increment_button = tk.Button(button_frame, text="+", command=lambda t=theta.lower().replace(" ", ""): self.increment_theta(t))
            increment_button.pack(side=tk.TOP)

            decrement_button = tk.Button(button_frame, text="-", command=lambda t=theta.lower().replace(" ", ""): self.decrement_theta(t))
            decrement_button.pack(side=tk.TOP)
        ...
        for param in ["X", "Y", "Z", "Angle A"]:
            ...
            increment_button = tk.Button(button_frame, text="+", command=lambda p=param.lower().replace(" ", ""): self.increment_xyz(p))
            increment_button.pack(side=tk.TOP)

            decrement_button = tk.Button(button_frame, text="-", command=lambda p=param.lower().replace(" ", ""): self.decrement_xyz(p))
            decrement_button.pack(side=tk.TOP)
    ...
    def create_buttons(self, font_style):
        ...
        # Move to Vision Position Button
        ...
        self.start_track_button = tk.Button(self.left_frame, text="Start Track",
                                            ...
                                            height=2, width=25, state=tk.DISABLED)  # Disabled initially
    ...
    def increment_theta(self, theta_name):
        try:
            ...
            increment = 1  # Increment by 1 degree
    ...
    def decrement_theta(self, theta_name):
        try:
            current_value = float(self.theta_entries[theta_name].get() or 0)
            decrement = 1  # Decrement by 1 degree
    ...
    def increment_xyz(self, param_name):
        try:
            ...
            increment = 0.1  # Increment by 0.1 cm
            new_value = current_value + increment
    ...
    def decrement_xyz(self, param_name):
        try:
            current_value = float(self.theta_entries[param_name].get() or 0)
            decrement = 0.1  # Decrement by 0.1 cm
            new_value = current_value - decrement
            self.theta_entries[param_name].delete(0, tk.END)
            self.theta_entries[param_name].insert(0, f"{new_value:.2f}")
            self.update_theta_from_xyz()
    ...
    def move_to_zero(self):
        # Send the command "f 90 90 -120 -60" to Arduino
        self.arduino.send_command("f 90 90 -120 -60")
        # Update the GUI entries
        self.theta_entries["theta1"].delete(0, tk.END)
        self.theta_entries["theta1"].insert(0, "90.00")
        self.theta_entries["theta2"].delete(0, tk.END)
        self.theta_entries["theta2"].insert(0, "90.00")
        self.theta_entries["theta3"].delete(0, tk.END)
        self.theta_entries["theta3"].insert(0, "-120.00")
        self.theta_entries["theta4"].delete(0, tk.END)
        self.theta_entries["theta4"].insert(0, "-60.00")
    ...
    def open_camera(self):
        ...
        self.tracking_enabled = False  # Ensure tracking is disabled initially
        self.start_track_button.config(state=tk.NORMAL)
        self.stop_track_button.config(state=tk.DISABLED)
        self.stopped = True  # Ensure robot is stopped
    ...
    def close_camera(self):
        ...
        self.tracking_enabled = False  # Ensure tracking is disabled
        self.start_track_button.config(state=tk.DISABLED)
        self.stop_track_button.config(state=tk.DISABLED)
        self.arduino.send_command("s")  # Stop the robot
        self.stopped = True  # Ensure robot is stopped
        ...
        while self.camera_running and not self.stop_event.is_set():
            ...
            if current_time - self.last_command_time > self.command_interval:
                if self.tracking_enabled:
                    ...
                    # Tracking is disabled, ensure robot is stopped
                    if not self.stopped:
                        print("Tracking disabled. Sending stop command.")
                        self.arduino.send_command("s")  # Stop the robot
                        self.stopped = True
                        self.last_command_time = current_time
