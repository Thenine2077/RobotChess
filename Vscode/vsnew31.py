    ...
    def send_command(self, command_str):
        try:
            if self.serial and self.serial.is_open:
                ...
                # Replace .encode() with bytes() constructor
...
class RoboticArmGUI:
    def __init__(self, root, arduino, kinematics):
        ...
        self.PIXELS_PER_CM = 23.50  # Updated calibration: pixels per centimeter
        ...
        self.command_interval = 0.4  # seconds
    ...
    def plot_robot_arm(self, theta1, theta2, theta3, theta4):
        ...
        for i in range(len(points)-1):
            x_points = [points[i][0]/10, points[i+1][0]/10]  # Convert mm to cm
            y_points = [points[i][1]/10, points[i+1][1]/10]
            z_points = [points[i][2]/10, points[i+1][2]/10]
            self.ax.plot(x_points, y_points, z_points, color='blue', linewidth=4)

        # Plot joints
        x_coords = [p[0]/10 for p in points]
        y_coords = [p[1]/10 for p in points]
        ...
        # Axis limits
        self.ax.set_xlim([-500/10, 500/10])  # Converted to cm
        self.ax.set_ylim([-500/10, 500/10])
        self.ax.set_zlim([0, 500/10])
    ...
    def detect_blue_rectangle(self, frame):
        ...
        # Updated Blue color range in HSV after calibration
        lower_blue = np.array([90, 50, 50])   # Adjusted lower hue
        upper_blue = np.array([130, 255, 255]) # Adjusted upper hue
        ...
        # Detect object
        frame, detected_object = self.detect_object(frame, mask_blue, is_circle=False)
    ...
    def detect_object(self, frame, mask, is_circle):
        ...
        for contour in contours:
            ...
            if area > 1000:  # Increased area threshold for better detection
                if is_circle:
                    # Existing circle detection logic
                    if len(contour) >= 5:
                    ...
                    if len(approx) == 4 and cv2.isContourConvex(approx):
                        ...
                        if 0.8 <= aspect_ratio <= 1.2:  # Adjusted aspect ratio for rectangles
...
def select_port_gui(root, arduino_ports):
    ...
    Button(top, text="Confirm", command=confirm_selection, width=20).pack(pady=20)
