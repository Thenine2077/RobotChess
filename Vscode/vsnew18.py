...
# Function to select Arduino port via GUI
...
# Constant variables for robotic arm lengths
OA = 122.75
AB = 53.10
BC = 104.05
CD = 194.71
...
def validate_all_thetas(theta_values):
    ...
    if invalid_thetas:
        error_message_label.config(text="Angle limit exceeded in " + ", ".join(invalid_thetas))
        return False
    else:
...
# Update theta values using Inverse Kinematics
...
# Move to x, y, z, a using Inverse Kinematics and send command to Arduino
def move_to_xyz():
    try:
        x_target = float(x_entry.get() or 0)
        y_target = float(y_entry.get() or 0)
